...
accountsRouter.get("/me", async (req: AuthenticatedRequest, res) => {
    try {
...
accountsRouter.get("/gmail/auth-url", (req: AuthenticatedRequest, res) => {
    const state = `${req.user!.uid}:${randomBytes(16).toString("hex")}`;
    res.json({ url: getGmailAuthUrl(state) });
});

accountsRouter.get("/hubspot/auth-url", (req: AuthenticatedRequest, res) => {
...
accountsRouter.post(
    ...
    async (req: AuthenticatedRequest, res) => {
        try {
            ...
            const message =
                err instanceof Error ? err.message : "Gmail connect failed";
            res.status(400).json({ error: message });
        }
    },
);

accountsRouter.delete("/gmail", async (req: AuthenticatedRequest, res) => {
    const account = await getAccountByUserId(req.user!.uid);
    if (!account) {
...
accountsRouter.patch("/filters", async (req: AuthenticatedRequest, res) => {
    const account = await getAccountByUserId(req.user!.uid);
    if (!account) {
        res.status(404).json({ error: "No account found" });
        return;
    ...
    const { includeDomains, excludeDomains } = req.body as {
...
accountsRouter.delete("/hubspot", async (req: AuthenticatedRequest, res) => {
    const account = await getAccountByUserId(req.user!.uid);
    if (!account) {
        res.status(404).json({ error: "No account found" });
        return;
