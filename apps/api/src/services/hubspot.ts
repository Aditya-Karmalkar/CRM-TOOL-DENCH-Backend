...
export async function exchangeHubspotCode(
    ...
    if (!account) throw new Error("Connect Gmail before Dench CRM");
...
export async function refreshHubspotTokensIfNeeded(
    accountId: string,
    tokens: StoredTokens,
): Promise<StoredTokens> {
    const buffer = 5 * 60 * 1000; // 5 minute buffer
    if (Date.now() < tokens.expiryDate - buffer) {
        return tokens;
    }

    console.log(`Refreshing expired HubSpot token for account ${accountId}...`);
    const body = new URLSearchParams({
    ...
    if (!res.ok) {
        throw new Error(`Dench CRM token refresh failed: ${await res.text()}`);
    }
    ...
    await setHubspotTokens(accountId, newTokens);
    return newTokens;
}

function hubspotClient(tokens: StoredTokens): Client {
...
export async function upsertContactAndNote(
    tokens: StoredTokens,
    email: ParsedEmail,
    ...
    const properties: Record<string, string> = { email: contactEmail };
    if (firstName) properties.firstname = firstName;
    if (lastName) properties.lastname = lastName;
    if (extracted.contact.phone) properties.phone = extracted.contact.phone;
    ...
    if (search.results[0]?.id) {
        ...
        await withRetry(() =>
            client.crm.contacts.basicApi.update(contactId, { properties }),
        );
    } else {
