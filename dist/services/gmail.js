...
export function createOAuth2Client() {
    return new google.auth.OAuth2(env.googleClientId, env.googleClientSecret, env.gmailRedirectUri);
}
...
export async function refreshGmailTokensIfNeeded(tokens) {
...
export async function registerGmailWatch(tokens, topicName) {
...
export async function stopGmailWatch(tokens) {
    const gmail = gmailClientFromTokens(tokens);
    await gmail.users.stop({ userId: 'me' });
}
export async function fetchMessage(tokens, gmailMessageId) {
    const gmail = gmailClientFromTokens(tokens);
...
export async function listHistoryMessages(tokens, startHistoryId) {
    const gmail = gmailClientFromTokens(tokens);
