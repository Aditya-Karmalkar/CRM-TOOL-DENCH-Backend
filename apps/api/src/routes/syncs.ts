...
syncsRouter.get("/", async (req: AuthenticatedRequest, res) => {
    try {
...
syncsRouter.post("/:id/retry", async (req: AuthenticatedRequest, res) => {
    try {
...
syncsRouter.post("/manual", async (req: AuthenticatedRequest, res) => {
    try {
        const maxMessages = Math.min(
            Number((req.body as { maxMessages?: number }).maxMessages) || 50,
            200,
        );
